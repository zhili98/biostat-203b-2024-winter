...
1. Final Results
    ...
    - As we can see from the table above, `Model Stacking` has the highest `accuracy` (0.6145227) and `ROC_AUC` (0.6587066) among all 4 models. `XGBoost` comes second with an `accuracy` of 0.6145227 and `ROC_AUC` of 0.6587066 which are pretty close to the results of the `Model Stacking`. `Random Forest` comes third with an `accuracy` of 0.6071985 and `ROC_AUC` of 0.6456452. `Logistic Regression` comes last with an `accuracy` of 0.5796234 and `ROC_AUC` of 0.6100015, which is not a satisfactory result as it did not show an obvious improvement compared to the baseline model.
